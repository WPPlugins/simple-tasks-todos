...
function ftm_get_tasks($args=NULL){
    ...
    // Build SQL
    $where = array();
    if(is_numeric($args['status'])) $where['status'] = "t.status = $args[status]";
    if(is_numeric($args['tlid'])) $where['tlid'] = "t.tlid = $args[tlid]";
    if(is_numeric($args['tid'])) $where['tid'] = "t.tid = $args[tid]";
